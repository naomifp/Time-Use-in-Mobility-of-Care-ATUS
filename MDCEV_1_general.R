...
### Set core controls
apollo_control = list(
  modelName    = "MDCEV_care_1_general",
  modelDescr   = "MDCEV model on ATUS with general categories of trip purposes and socio-demographics",
  indivID      = "id", 
...
apollo_beta = c(alpha_base              = -20,
                ...
                #care 
                delta_spouse_care	      = 0,
                delta_spousework_care	  = 0,
                delta_children_care	    = 0,
                delta_youngest_care	    = 0,
                delta_childtime_care	  = 0,
                delta_trvwchild_care	  = 0,
                delta_trvwspouse_care	  = 0,
                delta_trvwfam_care	    = 0,
                delta_eldertime_care  	= 0,
                delta_trvwelder_care  	= 0,
                delta_trvelderact_care  = 0,
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V = list()
  V[["outside"            ]] = 0
  V[["travel_care"        ]] = delta_travel_care    + delta_female_care * female + delta_spouse_care * spouse + delta_spousework_care * spouse * spousework + delta_children_care * children + delta_youngest_care * children * youngest + delta_childtime_care * children * childtime + delta_trvwchild_care * children * trv.care_w.hhchild + delta_trvwspouse_care * spouse * trv.care_w.spouse + delta_trvwfam_care * children * spouse * trv.care_w.hhchild * trv.care_w.spouse  + delta_eldertime_care * eldertime + delta_trvwelder_care * trv.care_w.elder + delta_trvelderact_care * eldertime * trv.care_w.elder   
  V[["travel_work"        ]] = delta_travel_work    + delta_female_work * female + delta_work_multijob * work * multijob +  delta_workFT * work * FTjob + delta_workPT * work * PTjob + delta_age_school * school * (age<25) + delta_school * school
  V[["travel_leisure"     ]] = delta_travel_leisure + delta_female_leisure * female    
